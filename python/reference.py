...
if config.optimize_eyeballpose and config.optimize_neckpose:
    ...
    torch.onnx.export(
        ...
        "flame.onnx",
        export_params=True,
        input_names=['shape', 'expression', 'pose', 'neck', 'eye'],
        output_names=['vertices', 'landmarks'],
    )

    import onnx
    onnx_model = onnx.load('flame.onnx')
    onnx.checker.check_model(onnx_model)

    print(vertice)  # [[3x f32], ...]
    print(landmark)
